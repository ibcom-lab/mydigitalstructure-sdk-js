...
mydigitalstructure._util.factory.search = function (param)
{
		              	...
		              	if (userFilter.type == 'text')
		              	{
							...
							[
								'<input id="util-view-search-user-filter-', userFilter.name, '" type="text"',
												' class="form-control myds-text input-sm myds-enter"',
							...
							'<div class="card">',
								'<div class="card-header">',
									'<div class="float-right" data-html2canvas-ignore="true">',
										'<button class="myds-click btn btn-white btn-sm" role="button" data-source="close" data-controller="' + context + '">' +
		               				'<i class="fa fa-times fa-fw"></i> Close' +
		              				'</button>',
		              				'<div class="btn-group" role="group">',
				                     '<button class="myds-click btn btn-white btn-sm ml-2" role="button" data-controller="', search.export.controller, '"',
				                     	...
				                     	'>',
			              				...
			              				'</button>',
			              				(search.charting?'<button class="myds-click btn btn-white btn-sm" role="button"' +
	...
	mydigitalstructure._util.controller.add(
	{
		...
		code: function (param)
		{
			var scope = mydigitalstructure._util.param.get(param.dataContext, 'scope').value;
			var context = mydigitalstructure._util.param.get(param.dataContext, 'context').value;
			var filename = mydigitalstructure._util.param.get(param.dataContext, 'filename').value;
	...
	app.add(
	{
		...
		{	
			...
			var scope = mydigitalstructure._util.param.get(param.dataContext, 'scope').value;
			var context = mydigitalstructure._util.param.get(param.dataContext, 'context').value;
			var name = mydigitalstructure._util.param.get(param.dataContext, 'name').value;
    ...
    mydigitalstructure._util.controller.add(
    {
        ...
        code: function (param)
        {
            ...
            var includeFixed = mydigitalstructure._util.param.get(param, 'includeFixed', {default: false}).value;
